...
- `fotometro:import-network` télécharge et importe les lignes, stations et accès réels depuis Île-de-France Mobilités (peut prendre plusieurs minutes). Le dataset `arrets-lignes` est téléchargé dans le répertoire temporaire configuré par `FOTOMETRO_IDFM_TEMP_DIR`, puis supprimé après succès lorsque `APP_DEBUG=false`.
