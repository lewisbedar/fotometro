<aside class="map-progress-panel map-glass" aria-labelledby="global-progress-title">
    ...
    <div class="mt-4 grid grid-cols-[72px_1fr] gap-4" x-show="! progressCollapsed || ! isSmallScreen" x-cloak>
        ...
        <dl class="space-y-2 text-sm">
            <div class="flex justify-between gap-4"><dt>Stations completes</dt><dd class="font-semibold">{{ $coverageStatusCounts['complete'] ?? 0 }}</dd></div>
            <div class="flex justify-between gap-4"><dt>Documentees</dt><dd class="font-semibold">{{ $coverageStatusCounts['documented'] ?? 0 }}</dd></div>
            <div class="flex justify-between gap-4"><dt>En cours</dt><dd class="font-semibold">{{ $coverageStatusCounts['in_progress'] ?? 0 }}</dd></div>
            <div class="flex justify-between gap-4"><dt>Planifiees</dt><dd class="font-semibold">{{ $coverageStatusCounts['planned'] ?? 0 }}</dd></div>
            <div class="flex justify-between gap-4"><dt>Non commencees</dt><dd class="font-semibold">{{ $coverageStatusCounts['not_started'] ?? 0 }}</dd></div>
        </dl>
        <p class="col-span-2 text-xs text-black/60">{{ $documentedStationCount }} stations documentees sur {{ $stationCount }}</p>
    </div>
</aside>
