<aside id="lines-panel" class="map-lines-panel map-glass" x-show="isLinesOpen" x-transition x-cloak aria-labelledby="lines-title">
    <div class="flex items-center justify-between">
        ...
        <button type="button" class="map-icon-button" x-on:click="isLinesOpen = false" aria-label="Fermer les lignes">×</button>
    </div>
        ...
        <template x-for="line in mapData.lines" :key="line.id">
            <button type="button" class="line-select-button" x-on:click="selectLine(line.id)" x-bind:class="{ 'is-active': selectedLineId === line.id }">
                ...
                <span class="min-w-0 flex-1 text-left">
                    <span class="block truncate font-semibold" x-text="line.name"></span>
                    <span class="block text-xs text-black/55"><span x-text="line.station_count ?? line.stations?.length ?? 0"></span> stations · <span x-text="line.progress?.percentage ?? 0"></span>%</span>
                </span>
            </button>
        </template>
    </div>
